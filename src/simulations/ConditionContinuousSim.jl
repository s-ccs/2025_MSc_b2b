...
struct ConditionContinuousDesign <: UnfoldSim.AbstractDesign
    n_trials::Int
    confound::Bool
    rho::Float64
end

UnfoldSim.size(design::ConditionContinuousDesign) = (design.n_trials,)
Base.length(design::ConditionContinuousDesign) = design.n_trials

function UnfoldSim.generate_events(
    rng::AbstractRNG,
    design::ConditionContinuousDesign
)
...
function make_cond_cont_components(cfg::ConditionContinuousConfig)
...
function make_cond_cont_onset(
    ...
    if !overlap
        max_component_length = maximum([
            ...
            #length(UnfoldSim.hanning(0.30, 0.35, cfg.sfreq))
        ])
...
function simulator_cond_cont(
    ...
    design = ConditionContinuousDesign(cfg.n_trials, confound, cfg.rho)
    components = make_cond_cont_components(cfg)
    onset = make_cond_cont_onset(cfg; overlap = overlap)
...
function simulate_cond_cont_cases(
    cfg::ConditionContinuousConfig;
    seed::Int = 12
)
    return (
        clean = simulator_cond_cont(cfg; confound = false, overlap = false, seed = seed),
        overlap = simulator_cond_cont(cfg; confound = false, overlap = true, seed = seed),
        confound = simulator_cond_cont(cfg; confound = true, overlap = false, seed = seed),
        both = simulator_cond_cont(cfg; confound = true, overlap = true, seed = seed)
    )
end
