...
# ╔═╡ 7d073d26-842b-11f1-aaef-dd87f544d8e7
begin
	using Random
	using Statistics
	using DataFrames
...
# ╔═╡ 0a8a2114-cd15-4c2d-98ad-d3b63dd8adf3
# =================
# design struct
# =================

begin
	UnfoldSim.@with_kw struct B2BToyDesign <: UnfoldSim.AbstractDesign
		n_trials::Int = 400
		confounded::Bool = false
		rho::Float64 = 0.8
	end

	UnfoldSim.size(design::B2BToyDesign) = (design.n_trials,)
	Base.length(design::B2BToyDesign) = design.n_trials
end

# ╔═╡ 7bc4a2cb-734b-4d82-9908-7de992a36ae9
# ==================
# onset function
# ==================
...
function make_onset(cfg; overlap = false)
	...
	target_mean_samples = target_mean_ms / 1000 * cfg.sfreq
...
# ╔═╡ 50b2741c-a7ea-4ca1-950d-2be508a0d240
# =====================
# generate events
# =====================


function UnfoldSim.generate_events(rng::UnfoldSim.AbstractRNG, design::B2BToyDesign)
...
# ╔═╡ 273b9483-73ff-4021-b779-39217d61e665
# =========================
# Standard Decoding
# =========================
...
begin
	...
	function get_target(evts, target::Symbol)
		if target == :condition
			# car = -1, face = +1
			return ifelse.(String.(evts.condition) .== "face", 1.0, -1.0)
		else
			return Float64.(evts[:, target])
		end
	end

	function fit_ridge(X, y; λ = 1.0)
		# X: trials × channels
		# y: trials
		...
		# standardize channels using training data
		μ = vec(mean(X; dims = 1))
		σ = vec(std(X; dims = 1))
		σ[σ .== 0.0] .= 1.0

		Xz = (X .- reshape(μ, 1, :)) ./ reshape(σ, 1, :)

		# add intercept
		Xd = hcat(ones(size(Xz, 1)), Xz)

		# ridge penalty, but do not penalize intercept
		p = size(Xd, 2)
		P = Diagonal(vcat(0.0, fill(λ, p - 1)))

		β = (Xd' * Xd + P) \ (Xd' * y)

		# return a simple NamedTuple, not struct
		return (β = β, μ = μ, σ = σ)
	end

	function predict_ridge(model, X)
		# X: trials × channels

		X = Matrix{Float64}(X)

		Xz = (X .- reshape(model.μ, 1, :)) ./ reshape(model.σ, 1, :)
		Xd = hcat(ones(size(Xz, 1)), Xz)

		return Xd * model.β
	end

	function decode_ridge(dat_3d, evts, target::Symbol; nfolds = 6, λ = 1.0)
		# dat_3d: channels × time × trials

		@assert ndims(dat_3d) == 3
		@assert size(dat_3d, 3) == nrow(evts)

		n_channels, n_times, n_trials = size(dat_3d)

		y = get_target(evts, target)

		train_test = MLJBase.train_test_pairs(
			MLJBase.CV(; nfolds = nfolds),
			1:n_trials,
		)

		yhat_all = fill(NaN, n_times, n_trials)

		for (train_idx, test_idx) in train_test
			for t in 1:n_times
				# channels × trials → trials × channels
				X_train = Matrix(permutedims(@view dat_3d[:, t, train_idx]))
				X_test = Matrix(permutedims(@view dat_3d[:, t, test_idx]))
				...
				yhat_all[t, test_idx] .= predict_ridge(model, X_test)
			end
		end

		r = [
			std(yhat_all[t, :]) == 0 ? NaN : cor(yhat_all[t, :], y)
			for t in 1:n_times
		]
...
# ╔═╡ 9beaef43-1ff2-41e5-b31f-e6a23a6d11aa
# ==================
# config 
# ==================
...
begin
	Base.@kwdef struct ToyConfig
		...
		# condition-continuous confound strength
		rho::Float64 = 0.8
	end
end

# ╔═╡ d03bcea6-02e9-41e9-bee8-227743863bc0
# ==================
# make components
# =================

function make_components(cfg::ToyConfig)
...
# ╔═╡ 26cd8289-037b-48e0-9886-5854a1beabd3
# ===================
# simulate one case
# ===================
...
# ╔═╡ eeda6e75-ab6b-4bb8-bb76-be1153176cba
begin
	md"""
...
# ╔═╡ 2e3b1fcb-a1e7-491c-ba10-f49fd34b47f6
cfg = ToyConfig(
	rho = rho_slider,
...
# ╔═╡ 1f9fb091-aa0d-4c10-86e9-4b907c3c2acb
# =========================
# Plot
# ==========================

function plot_decoding_scores(
	scores;
	title = "Standard ridge decoding",
	ylabel = "Decoding score r",
	...
	# -------------------------
	n170_gt = UnfoldSim.n170(; sfreq = cfg_plot.sfreq)
	p300_gt = UnfoldSim.p300(; sfreq = cfg_plot.sfreq)

	n170_peak = argmax(abs.(n170_gt))
	p300_peak = argmax(abs.(p300_gt))

	figure_height = show_intercepts ? 650 : 400

	f = Figure(size = (900, figure_height))
	...
	ax = Axis(
		f[1, 1],
		title = title,
		xlabel = show_intercepts ? "" : "Time [samples]",
		ylabel = ylabel,
		xgridvisible = false,
		ygridvisible = false,
	)
	...
	for case_name in case_order
		sub = scores[scores.case .== case_name, :]
	...
	# =========================
	# Bottom: true components
	# =========================
	if show_intercepts
		ax_components = Axis(
			f[2, 1],
			title = "Ground-truth component coefficients",
			xlabel = "Time [samples]",
		...
		n170_intercept =
			cfg_plot.β0_n170 .* n170_gt

		n170_condition_effect =
			cfg_plot.β_condition .* n170_gt

		p300_intercept =
			cfg_plot.β0_p300 .* p300_gt

		p300_continuous_effect =
			cfg_plot.β_continuous .* p300_gt

		lines!(
			ax_components,
			1:length(n170_intercept),
			n170_intercept;
		...
		lines!(
			ax_components,
			1:length(n170_condition_effect),
			n170_condition_effect;
			label = "N170 condition effect",
			linestyle = :dash,
		)

		lines!(
			ax_components,
			1:length(p300_intercept),
			p300_intercept;
		...
		lines!(
			ax_components,
			1:length(p300_continuous_effect),
			p300_continuous_effect;
			label = "P300 continuous effect",
			linestyle = :dash,
		)
...
# ╔═╡ 2a56c2ff-e5f2-4fca-8e5a-c8476df1604b
# ================
# check betas
# ===================
(
...
# ╔═╡ e40b2cc5-b46a-48c3-b4bb-93b04edea145
begin
	# ==========================
	# four cases
	# ==========================
	
	dat_clean, evts_clean =
		simulate_case(cfg; confounded = false, overlap = false, seed = 12)
	
	dat_overlap, evts_overlap =
		simulate_case(cfg; confounded = false, overlap = true, seed = 12)
	
	dat_confound, evts_confound =
		simulate_case(cfg; confounded = true, overlap = false, seed = 12)
	
	dat_both, evts_both =
		simulate_case(cfg; confounded = true, overlap = true, seed = 12)
end

# ╔═╡ 5c0450ef-060a-4bc8-ac19-f2f6ac6432c7
# ==================
# check correlation
# ==================

begin
	function condition_to_num(evts)
		ifelse.(evts.condition .== "face", 1.0, -1.0)
	end

	cor(condition_to_num(evts_confound), evts_confound.continuous)
	# cor(condition_to_num(evts_clean), evts_clean.continuous)
end

# ╔═╡ 9e615de2-ef86-42a8-8f57-d792f81ebad0
# ===================
# run four cases
# =================
...
begin
	cases = (
	clean = (dat_clean, evts_clean),
	overlap = (dat_overlap, evts_overlap),
	confound = (dat_confound, evts_confound),
	both = (dat_both, evts_both),
)

	function decode_cases(
	cases,
	target::Symbol;
	nfolds = 6,
	λ = 1.0,
	...
	all_yhats = Dict{Symbol, Matrix{Float64}}()

	for (case_name, (dat, evts)) in pairs(cases)

		score, yhat = decode_ridge(
			dat,
			evts,
			target;
			nfolds = nfolds,
		...
		score[!, :case] = fill(string(case_name), nrow(score))

		append!(all_scores, score)

		all_yhats[case_name] = yhat
	end
			
	return all_scores, all_yhats
end
end

# ╔═╡ d477da23-2a7b-4257-87f8-8cc35b2f9854
# ====================
# condition decoding
# ====================

begin
	score_condition, yhat_condition =
	decode_cases(
		cases,
	...
	score_condition;
	title = "Standard ridge decoding: condition",
	ylabel = "cor(predicted condition, true condition)",
)
end

# ╔═╡ 8c9d832e-e96b-40ae-b3c7-5ef2fe1de090
# ======================
# continuous decoding
# =====================

begin
	score_continuous, yhat_continuous =
	decode_cases(
		cases,
		:continuous;
		nfolds = 6,
	...
	plot_decoding_scores(
	score_continuous;
	title = "Standard ridge decoding: continuous",
	ylabel = "cor(predicted continuous, true continuous)",
)
...
manifest_format = "2.0"
project_hash = "a055c081c0afebf2c3e2a13223b030b244a26889"
...
git-tree-sha1 = "a648a4cb5c2e5ae8bf2f6ff1f6985f4c6ede2e8d"
...
version = "0.10.9"
weakdeps = ["Arrow", "JSON", "RecipesBase", "SentinelArrays", "StructTypes"]
...
[[deps.UnfoldDecode]]
deps = ["DataFrames", "LinearAlgebra", "Logging", "MLBase", "MLJ", "MLJLinearModels", "MLJMultivariateStatsInterface", "MultivariateStats", "ProgressMeter", "Tables", "Unfold", "XGBoost"]
git-tree-sha1 = "6a9ad6790f90affc5c88467ab508140a8042605e"
uuid = "ec0f67a1-ae9f-4687-b20b-bd39d33e72da"
version = "0.1.1"
