...
# ╔═╡ 1d569eb6-4d74-11f1-8f7a-75ab2e270cfb
begin
	using DataFrames, Random, Distributions, Statistics, UnfoldSim, WGLMakie, PlutoUI
	
	Base.@kwdef struct EventTableDesign <: UnfoldSim.AbstractDesign
	    events::DataFrame
	    shuffle_events::Bool = true
	end
	
	Base.size(d::EventTableDesign) = (nrow(d.events),)
	UnfoldSim.size(d::EventTableDesign) = nrow(d.events)
	
	function UnfoldSim.generate_events(rng::Random.AbstractRNG, d::EventTableDesign)
	    if d.shuffle_events
	        return d.events[shuffle(rng, 1:nrow(d.events)), :]
	    else
	        return copy(d.events)
	    end
	end
end

# ╔═╡ 54872737-5b5e-4a86-b4e6-96ad0c51ace1
function make_collinear_events(rng;
    n_trials = 400,
    collinearity_strength = 0.0,
)

    @assert 0.0 <= collinearity_strength <= 1.0
    @assert iseven(n_trials)

    n_each = div(n_trials, 2)
    ...
    # collinearity_strength = 0:
    # car and face have similar continuous values
    #
    # collinearity_strength = 1:
    # car mostly low continuous, face mostly high continuous
    Δ = 3.0 * collinearity_strength

    continuous_car =
        2.5 .- Δ / 2 .+ randn(rng, n_each)

    continuous_face =
        2.5 .+ Δ / 2 .+ randn(rng, n_each)

    continuous = clamp.(
        vcat(continuous_car, continuous_face),
        0,
        5,
    )
    ...
    return events[shuffle(rng, 1:n_trials), :]
end

# ╔═╡ 077ef9fa-f952-48f9-b503-d0fbca9a2616
# ╠═╡ disabled = true
#=╠═╡
function make_collinear_design(rng;
    n_trials = 400,
    collinearity_strength = 0.0,
    shuffle_events = true,
)
    events = make_collinear_events(
        rng;
        n_trials = n_trials,
        collinearity_strength = collinearity_strength,
    )

    return EventTableDesign(
        events = events,
...
# ╔═╡ b34e68e2-21fb-4104-b377-68377b8f9ce8
# ╠═╡ disabled = true
#=╠═╡
...
# ╔═╡ b2932513-9996-4e1f-beb5-656839788cdb
begin
    events_low = make_collinear_events(
        MersenneTwister(1);
        n_trials = 400,
        collinearity_strength = 0.0,
    )

    events_mid = make_collinear_events(
        MersenneTwister(1);
        n_trials = 400,
        collinearity_strength = 0.5,
    )

    events_high = make_collinear_events(
        MersenneTwister(1);
        n_trials = 400,
        collinearity_strength = 1.0,
    )
end

# ╔═╡ d8c7bf71-2c63-473a-be9e-0dd060df2157
function condition_continuous_cor(events)
    condition_numeric = Float64.(events.condition .== "face")
    return cor(condition_numeric, events.continuous)
end

# ╔═╡ 71ac194e-d560-4929-92af-585a5e7a93ef
begin
    DataFrame(
        setting = ["low", "mid", "high"],
        collinearity_strength = [0.0, 0.5, 1.0],
        correlation = [
            condition_continuous_cor(events_low),
            condition_continuous_cor(events_mid),
            condition_continuous_cor(events_high),
        ],
    )
end

# ╔═╡ 8ab9ce49-f4dc-4bb1-ac53-7a66099f20b8
function plot_continuous_by_condition(events; title = "")
    ...
    x = ifelse.(events.condition .== "car", 1.0, 2.0)
    ...
    ax.xticks = ([1, 2], ["car", "face"])

    fig
end

# ╔═╡ 431f596d-6307-4afc-8000-39b37aa9a58f
plot_continuous_by_condition(
    events_low;
    title = "Low collinearity: condition and continuous are almost independent",
)

# ╔═╡ 89ce18d2-58cf-4515-85e7-e45cbc80ff38
plot_continuous_by_condition(
    events_high;
    title = "High collinearity: face has larger continuous values",
)

# ╔═╡ 5fa78dcb-76e3-4e18-9666-b5766f17ea79
begin
    strengths = 0.0:0.1:1.0

    cors = [
        condition_continuous_cor(
            make_collinear_events(
                MersenneTwister(1);
                n_trials = 400,
                collinearity_strength = s,
            )
        )
        for s in strengths
    ]
    ...
    ax = Axis(
        fig[1, 1],
        xlabel = "collinearity_strength",
        ylabel = "cor(condition == face, continuous)",
        title = "Collinearity knob check",
    )

    lines!(ax, strengths, cors)
    scatter!(ax, strengths, cors)

    fig
end

# ╔═╡ 3f5ff23b-5cb3-4818-bc8f-cdb432fb0cae
# ╠═╡ disabled = true
#=╠═╡
@bind overlap_strength Slider(0:0.1:1; default = 1.0, show_value = true)
@bind collinearity_strength Slider(0:0.1:1; default = 0.0, show_value = true)
  ╠═╡ =#

# ╔═╡ 5d894b6f-a78e-4c6a-ad4b-092c17c2bbbf
# ╠═╡ disabled = true
#=╠═╡
@bind scenario Select([
    "low_overlap_low_collinearity",
    "low_overlap_high_collinearity",
    "high_overlap_low_collinearity",
    "high_overlap_high_collinearity",
])
  ╠═╡ =#

# ╔═╡ 8ca71298-1ce6-4515-90e6-5d8ded985320
# ╠═╡ disabled = true
#=╠═╡
begin
    if scenario == "low_overlap_low_collinearity"
        overlap_strength = 0.0
        collinearity_strength = 0.0
    elseif scenario == "low_overlap_high_collinearity"
        overlap_strength = 0.0
        collinearity_strength = 1.0
    elseif scenario == "high_overlap_low_collinearity"
        overlap_strength = 1.0
        collinearity_strength = 0.0
    elseif scenario == "high_overlap_high_collinearity"
        overlap_strength = 1.0
        collinearity_strength = 1.0
    end
end
...
manifest_format = "2.0"
project_hash = "851bc216a5f52bbd39dc8fcd285c6a25f57c6d37"
