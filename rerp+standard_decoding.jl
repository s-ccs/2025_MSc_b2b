...
begin
	...
	Pkg.add(["UnfoldSim","UnfoldMakie","CairoMakie","Unfold","MLJ","MultivariateStats","MLJMultivariateStatsInterface","PlutoUI", "DataFrames", "StatsModels",
			])
end


# ╔═╡ e73a39d5-0fc2-49ee-992f-cbcada9b1d88
...
# ╔═╡ 0374e125-7f41-49c4-9fb9-3bf5b6327eec
...
# ╔═╡ 1b4d3609-c3d6-43c3-bb60-5c30a6534df2

	# =========================
	# rERP design
	# ==========================
	function make_rerp_design(
		cfg::Decoding_Config;
		event_name::AbstractString = "stimulus",
	)
		basis = Unfold.firbasis(
			τ = [-0.1, 1.0],
			sfreq = cfg.sfreq,
			name = event_name,
		)
...
function fit_rerp_cases(
	...
	model;
	target::Symbol = :continuous,
	event_name::String = "stimulus",
	nfolds::Int = 2,
)
    scores_tables = DataFrame[]
    fitted_models = Dict{Symbol, Any}()

    for (case_name, case_data) in pairs(cases)

        dat_cont, evts = case_data
		...
		score_case = coeftable(uf_rerp; measure = RSquared(), averaged = true,)
...
# ╔═╡ 64346342-5055-4f4b-8a55-7c266413bd8a
# ╠═╡ disabled = true
#=╠═╡
begin
...
function plot_decoding_grid(
    ...
    ylabel_text =
        if score_col == :r
            "Cross-validated correlation r"
        elseif score_col in (:estimate, :r_squared)
            "Cross-validated R²"
        else
            string(score_col)
        end
